...
5. nodemon.json for development perpose
6. ecosystem.json for production perpose pm2 setup etc

7. commitlint setup
8. npm i @commitlint/cli @commitlint/config-conventional -D
9. make "commit-msg" file in husk folder and add
   ` #!usr/bin/env sh
   . "$(dirname -- "$0")/\_/husky.sh"

npx --no-install commitlint --edit "$1" ` 3) make commitlint.config.js on root directory
add document to commitlint.config.js 4) now you have add prefix

4. eslint setup
1. npm i --save-dev @eslint/js @types/eslint\_\_js typescript-eslint
1. add eslint.config.mjs
1. npx eslint .
1. add pre-commit script
...
1. npm i --save-dev eslint-config-prettier
1. add scripts in package.json like eslint

1. dotenv-flow
1. npm i dotenv-flow cross-env
1. change dev script to " cross-env NODE_ENV=development nodemon src/server.ts"
1. change start script to " cross-env NODE_ENV=production node dist/server.js"
   make sure you have .env.development & .env.production
1. add config files config.ts
...
1. npm i winston
1. create logger
1. add logs file outside src directory
